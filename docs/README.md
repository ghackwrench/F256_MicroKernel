...
Events belong to one of three categories: **Solicited Events**, **IRQ Events**, and **Unsolicited Events**. 
...
Starting with the January 2025 kernel releases, programs can receive events in
response to interrupts generated by devices that aren't otherwise in-use by
the kernel. The kernel always claims the RTC, SOF, and PS2 interrupts. If the
SLIP dip-switch is on, it also claims the UART interrupts. By default, the
kernel masks all other interrupt sources at the interrupt controller.

To enable interrupt events for any other device, simply unmask its matching
interrupt controller bit.  Once this is done, when an interrupt comes in from
the device, the kernel will clear its pending bit in the interrupt controller
and queue an **event.irq.IRQ** event with the following properties:

* **event.irq.group** contains the interrupt's group number (0 or 1 for REG0 or REG1).
* **event.irq.bitval** contains the value of the interrupt's bit (1, 2, 4, ... 128).

NOTE: If your program falls behind and the kernel runs out of events objects,
the kernel's handler will respond by re-masking the offending interrupt source
(this only applies to user-enabled sources).

On a related note, prior to January 2025, if a non-kernel device raised an
interrupt, the kernel would automatically respond by clearing the associated
pending bit. This is no longer done. Instead, the kernel will only respond to
interrupts that are unmasked. This change allows programs to safely poll/clear
the interrupt controller's pending bits instead of using the IRQ Events
feature.
